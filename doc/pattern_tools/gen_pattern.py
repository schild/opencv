...
class PatternMaker:
    ...
    def make_charuco_board(self):
        ...
        if ch_ar_border < side*0.7:
            print(
                f"Marker border {ch_ar_border} is less than 70% of ArUco pin size {int(side)}. Please increase --square_size or decrease --marker_size for stable board detection"
            )
        marker_id = 0
        for y in range(0, self.rows):
            for x in range(0, self.cols):
                ...
                if x % 2 == y % 2:
                    ...
                    img_mark = self._create_marker_bits(
                        markerSize_bits, dictionary[f"marker_{str(marker_id)}"]
                    )
                    marker_id +=1
